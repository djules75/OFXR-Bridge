# OFXR Bridge v0.2.3_beta4 (V277) — steadier delivery on SteamVR
...
over. Image quality, settings and the VDXR/Oculus/PimaxXR paths are untouched.
...
The overlay's delivered figure should sit closer to your headset's refresh rate
...
They come from **one machine, one headset and one title** — Hogwarts Legacy
through UEVR on SteamVR with a Pimax Crystal Super OLED. The faults fixed are
structural and not specific to that setup, but the size of the improvement on
yours may differ. Reports either way are useful.
