...
struct LauncherSettings {
    ...
    int grid_phase_us{};
    // How far the pair's two frames are spaced away from one period, in
    // microseconds; negative leaves the layer's own controller in charge.
    // Zero is a real value here, not "unset" - it is the only spacing where
    // both intervals of a pair are exactly one period, and it is the first
    // thing worth trying. Diagnostic, for the same reason as grid_phase_us.
    int grid_bias_us{-1};
};
